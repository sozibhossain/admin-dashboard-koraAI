...
export default function InboxPage() {
  ...
  // mobile: which pane is showing. Above lg both panes are always visible.
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: () => {
      // Server will broadcast via socket; we still refetch as a safety net.
  ...
  // Mark conversation read when opened (only if there are unread messages).
  useEffect(() => {
  ...
  useInboxSocket(selectedId, {
    onNewMessage: (payload: any) => {
      ...
      // Refresh conversation list (last message + unread)
      queryClient.invalidateQueries({ queryKey: ["inbox-conversations"] });
      // If user is viewing this thread, append message directly for snappy UX.
  ...
  const handleStartConversation = (peer: any) => {
    ...
    // Find existing conversation with this peer; otherwise prepare a "virtual" one.
    ...
    // Optimistically inject a placeholder conversation so the chat panel becomes interactive
    const placeholderId = `pending-${peer._id}`;
    queryClient.setQueryData(["inbox-conversations"], (current: any) => {
      const list = current?.data || [];
      if (list.some((c: any) => String(c._id) === placeholderId)) {
        return current;
      }
      const placeholder = {
        _id: placeholderId,
  ...
  // After first send to a pending conversation, swap the placeholder for the real id.
  const handleSend = () => {
    if (!canSend) return;
  ...
  return (
    <div>
      <Header
        title="Inbox"
        subtitle="Live chat between admin and sales partners."
        action={
      ...
      <div className="p-3 sm:p-4 lg:p-6">
        ...
        >
              ...
              {conversationsLoading ? (
                ...
                <div className="p-6 text-center text-xs text-gray-500">
                  {search || tab === "unread"
                    ...
                    : "No conversations yet. Click + to start one with a sales partner."}
                </div>
              ) : (
            ...
            <div className="border-t border-[#1e2d40] px-5 py-3">
              {selectedConversation?.isGroup ? (
                <p className="text-xs text-gray-500">
                  Group sending is not yet wired in the API.
                </p>
              ) : (
                <div className="flex items-center gap-2">
                  <Input
                    placeholder={
                      recipient ? `Message ${recipient.name}...` : "Select a conversation first"
                    }
                    value={draft}
          ...
          {/* RIGHT — contact details (xl+ only; on smaller screens this lives in a future bottom sheet) */}
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex justify-between border-b border-[#1e2d40] py-1.5 last:border-0">
      ...
      <span className="max-w-[140px] truncate text-right text-gray-200">{children}</span>
    </div>
  );
}
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <div className="relative">
          <Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-gray-500" />
          <Input
            placeholder="Search sales partners..."
            value={query}
        ...
        <div className="max-h-[400px] space-y-1 overflow-y-auto">
          {isLoading ? (
            ...
            <p className="py-6 text-center text-xs text-gray-500">No partners found.</p>
          ) : (
